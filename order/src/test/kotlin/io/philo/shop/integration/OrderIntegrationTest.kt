...
//        val itemClient = mockk<ItemRestClientFacade>()
        every { itemClient.requestItems(any()) } returns
                listOf(ItemInternalResponse(1L, "드로우핏 캐시미어 발마칸 코트 D.NABY", "M", 190_000))
        every { orderEventPublisher.publishEvent(any()) } just runs

        val requestBody = OrderCreateRequest(orderLineRequests = listOf(OrderLineRequest(itemId = 1L, quantity = 5)))
        ...
        val orderItems = foundEntity.orderItems
