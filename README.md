...
| 모듈 명        | 한글 뜻      | 설명                                                                                   |
|-------------|-----------|--------------------------------------------------------------------------------------|
| eureka      | 서비스 레지스트리 | 마이크로 서비스를 이름을 통해 실제 IP, Port를 찾을 수 있게 한다                                             |
| api-gateway | 게이트 웨이    | 각 마이크로 서비스를 라우팅하고 인가/로깅 등의 부가 기능을 수행한다 (reverse-proxy)                               |
...
- 분해 전략: DDD의 `sub-domain`과 `bounded-context`를 통해 마이크로 서비스를 분리하고자 함
- 성능
